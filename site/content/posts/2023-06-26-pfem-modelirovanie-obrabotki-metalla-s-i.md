---
author: Yury Novozhilov
date: 2023-06-26 15:02:19+00:00
layout: post
link_previews:
- description: Ultrasonic vibration-assisted machining (UVAM) incorporates high-frequency
    vibrations during cutting, providing advantages such as thinner chips, improved
    surface treatment, and lower cutting forces. From a computational perspective,
    simply adding oscillatory load to simulations does not impact the results significantly.
    However, accurately incorporating acoustic-induced softening phenomena into constitutive
    modeling produces realistic outcomes, including thinner chips, improved chip formation,
  image: https://static.licdn.com/aero-v1/sc/h/c45fy346jw096z9pbphyyhdz7
  title: '#machining #chip #ultrasonic #fem | Hadi Bakhshan | 37 comments'
  url: https://www.linkedin.com/posts/hadi-bakhshan-aab4aa90_machining-chip-ultrasonic-activity-7077640666654076930-xBaY/
- description: 'PFEM development for the kinetics of fluids and solids. Modelling
    of subtractive manufacturing processes. G-PFEM: Particle Finite Element Method
    platform for geotechnical applications.'
  image: https://gitlab.com/uploads/-/system/project/avatar/10733668/pfem-kratos_logo.png
  title: PFEM / Pfem-Kratos · GitLab
  url: https://gitlab.com/pfem-research/kratos
- description: Ultrasonic vibration-assisted machining (UVAM) incorporates high-frequency
    vibrations during cutting, providing advantages such as thinner chips, improved
    surface treatment, and lower cutting forces. From a computational perspective,
    simply adding oscillatory load to simulations does not impact the results significantly.
    However, accurately incorporating acoustic-induced softening phenomena into constitutive
    modeling produces realistic outcomes, including thinner chips, improved chip formation,
  image: https://static.licdn.com/aero-v1/sc/h/c45fy346jw096z9pbphyyhdz7
  title: '#machining #chip #ultrasonic #fem | Hadi Bakhshan | 37 comments'
  url: https://www.linkedin.com/posts/hadi-bakhshan-aab4aa90_machining-chip-ultrasonic-activity-7077640666654076930-xBaY
source: vk
title: PFEM моделирование обработки металла с использованием ультразвуковых колебаний
---

Обработка с использованием ультразвуковых колебаний (UVAM) включает в себя высокочастотные колебания во время резания, обеспечивая такие преимущества, как более тонкая стружка, улучшенная обработка поверхности и меньшие силы резания. С точки зрения вычислений, простое добавление колебательной нагрузки к моделированию не оказывает существенного влияния на результаты. Однако точное включение явлений размягчения, вызванных акустическим воздействием, в моделирование структуры позволяет получить реалистичные результаты, включая более тонкую стружку, улучшенное формирование стружки и снижение силы резания.

https://www.linkedin.com/posts/hadi-bakhshan-aab4aa90_machining-chip-ultrasonic-activity-7077640666654076930-xBaY/

Коллеги выполнили расчет при помощи метода конечных элементов с частицами (PFEM) для моделирования, как обычной обработки, так и обработки с использованием ультразвуковых колебаний (UVAM). Считали в академическом коде Kratos PFEM https://gitlab.com/pfem-research/kratos

Хотя меня больше интересует математика их модели материала, которая учитывает подобное ультразвуковое воздействие.

[Hadi Bakhshan on LinkedIn: #machining #chip #ultrasonic #fem | 34 comments](https://www.linkedin.com/posts/hadi-bakhshan-aab4aa90_machining-chip-ultrasonic-activity-7077640666654076930-xBaY)
https://www.linkedin.com/posts/hadi-bakhshan-aab4aa90_machining-chip-ultrasonic-activity-7077640666654076930-xBaY/
https://gitlab.com/pfem-research/kratos
